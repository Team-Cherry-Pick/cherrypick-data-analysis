...
def _calculate_success_rate() -> float:
    total = get_crawler_data(Site.PPOMPPU, DataKey.TOTAL_COUNT)
    fail = get_crawler_data(Site.PPOMPPU, DataKey.FAILURE_COUNT)
...
st.title("📊 PPOMPPU CRAWLER DASHBOARD")
...
# ✅ 대시보드 본문
status = get_crawler_status(Site.PPOMPPU)
...
with col1:
    if st.button("▶️ START", use_container_width=True):
        set_crawler_status(Site.PPOMPPU, Status.RUNNING)
with col2:
    if st.button("⏹ STOP", use_container_width=True):
        set_crawler_status(Site.PPOMPPU, Status.STOPPED)
    ...
    if not st.session_state.break_confirm:
        ...
        with confirm_col1:
            if st.button("✅ 예, 중단합니다"):
                set_crawler_status(Site.PPOMPPU, Status.BREAK)
                st.session_state.break_confirm = False
        with confirm_col2:
            if st.button("❎ 취소"):
                st.session_state.break_confirm = False
...
# 현재 저장된 딜레이 불러오기
current_delay = get_crawler_data(Site.PPOMPPU, DataKey.DELAY_TIME)
    ...
    with delay_col2:
        if st.button("💾 딜레이 적용", use_container_width=True):
            try:
                ...
                set_crawler_data(Site.PPOMPPU, DataKey.DELAY_TIME, delay_input)
...
with col1:
    st.metric("📅 시작 시간", get_crawler_data(Site.PPOMPPU, DataKey.START_TIME))
with col2:
    avg = calculate_average_duration(Site.PPOMPPU)
    st.metric("⚡ 평균 소요 시간", f"{avg:.2f}s" if avg else "N/A")
with col3:
    st.metric("📦 총 수집 데이터", get_crawler_data(Site.PPOMPPU, DataKey.TOTAL_COUNT))

col4, col5, col6 = st.columns([1, 1, 1])

with col4:
    st.metric("❌ 실패한 데이터", get_crawler_data(Site.PPOMPPU, DataKey.FAILURE_COUNT))
with col5:
    st.metric("✅ 현재 페이지", f"{get_crawler_data(Site.PPOMPPU, DataKey.NOW_CRAWLING)}p")
with col6:
